            ...
            child: FocusTraversalGroup(
              ...
              child: Column(
                children: [
                  ...
                  // 创建一个Expander组件，用于展示和隐藏一些可选的信息
                  Expander(
                    // 当Expander打开时，标题的形状为一个没有圆角的矩形
                    headerShape: (open) => const RoundedRectangleBorder(
                      borderRadius: BorderRadius.zero,
                    ),
                    // Expander的标题为"Optional info"
                    header: const Text('Optional info'),
                    // Expander的内容为一个Row组件
                    content: Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        // Row的第一个子组件为一个Expanded组件，它占据Row的一半的空间
                        // 这个Expanded组件包含一个Markdown编辑器
                        Expanded(
                          flex: 2,
                          child: mt.Padding(
                            // 为Expanded组件添加一些内边距
                            padding:
                                const EdgeInsets.fromLTRB(2.0, 10.0, 15.0, 2.0),
                            child: Column(
                              children: [
                                // 第一个子组件是一个MarkdownToolbar，用于编辑Markdown文本
                                MarkdownToolbar(
                                  width: 45.0,
                                  height: 32.0,
                                ...
                                // 添加一个分隔线
                                const mt.Divider(),
                                // 第二个子组件是一个RawKeyboardListener，它监听键盘事件
                                // 当用户按下tab键时，它会在文本框中插入一个tab字符
                                RawKeyboardListener(
                                  focusNode: FocusNode(),
                                  ...
                                  // RawKeyboardListener的子组件是一个TextField，用户可以在这里输入文本
                                  child: mt.TextField(
                                    ...
                                    decoration: mt.InputDecoration(
                                      hintText: '输入...',
                                      hintStyle: getFontStyle(size: 18.0),
                                      labelText: 'detail text',
                                      border: const mt.OutlineInputBorder(),
                                      enabledBorder:
                        ...
                        // Row的第二个子组件也是一个Expanded组件，它也占据Row的一半的空间
                        // 这个Expanded组件包含一个Markdown预览器
                        Expanded(
                          flex: 2,
                          child: Padding(
                            padding:
                            ...
                            child: Column(
                              children: [
                                ...
                                // 使用Consumer来获取CommonData对象，然后将markdownTxt传递给MyMarkdownView进行渲染
                                Consumer(
                                  builder: (context, CommonData value, child) {
  ...
  static editContent(Content content, BuildContext context,
    ...
    if (onlyEdit) {
      ...
      if (old.category.name != ret.category.name) {
        ...
        // 检查新的分类是否已经存在
        if (n == null) {
          // 如果不存在，那么设置新的分类的id为自动增长的id，并将数量设置为0
          ret.category.id = Isar.autoIncrement;
          ret.category.count = 0;
        } else {
          // 如果存在，那么设置新的分类的id和数量为已存在的分类的id和数量
